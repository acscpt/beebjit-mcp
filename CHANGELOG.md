...
- **`reset` MCP tool**: hard-reset the BBC without destroying the session, equivalent to a user pressing BREAK on the real keyboard. The 6502 cycle counter wraps, BASIC state is cleared, and the boot banner reappears, the MCP session id stays valid. `autoboot=true` holds SHIFT across the BREAK so an inserted disc's `!BOOT` runs (the BBC SHIFT+BREAK convention). Driven by injecting an F12 keypress at the matrix to assert the 6502 RESET line, then advancing the CPU through an open-loop cycle window long enough for MOS init to settle on every supported model.

- **`load_disc` and `boot_disc` MCP tools**: mid-session disc handling. `load_disc(session_id, disc, drive=0, writeable=False, mutable=False)` mounts a disc image into drive 0 or 1 via beebjit's runtime disc-mount debugger command. `boot_disc` is the mount + SHIFT+BREAK autoboot one-shot. `writeable` cuts the write-protect notch; `mutable` flushes BBC writes back to the host file and requires `writeable`. Disc paths are checked Python-side before the debugger sees them, so a typo surfaces as a clean `FileNotFoundError`. Binaries that lack the runtime disc-mount command surface a `BeebjitError` pointing at the install docs.

- **`create_machine` cold-boot autoboot**: `create_machine(disc=...)` now holds SHIFT in the keyboard matrix from cycle zero, mounts the disc image into drive 0, and lets MOS init read SHIFT during its boot keyboard scan to dispatch autoboot. The gesture matches a real user holding SHIFT before powering on with a disc inserted. The argv-time `-autoboot` flag is no longer used. Side effect: `reset(autoboot=False)` on a disc-mounted session now reliably lands at the BASIC prompt; the previously-documented stickiness is gone.

- **Multi-model support**: `create_machine(model=...)` accepts `"b"` (BBC B with MOS 1.20, the default), `"master"` (Master 128 with MOS 3.20), `"mos35"` (Master 128 with MOS 3.50), and `"compact"` (Master Compact). The autoboot, screenshot, MODE 7 capture, register read, and disc handling tools all work uniformly across the four models. Disc-image format follows the model: BBC B and the two Master 128 variants read DFS images (`.ssd` and `.dsd`); Master Compact reads ADFS images (`.adl` and `.adf`).
...
- **`reset(autoboot=True)` actually fires `!BOOT`.** The previous implementation released SHIFT immediately after F12 release, before MOS's keyboard-matrix scan window. Calls that mounted a disc via `boot_disc` then invoked autoboot ended up at the bare cold-boot BASIC prompt because MOS never saw SHIFT held during its autoboot detection. `reset()` now holds SHIFT across the BREAK and through a generous post-BREAK open-loop cycle window long enough to cover MOS init's autoboot keyboard scan on every supported model. Verified end-to-end with a Nightworld smoke that fires `!BOOT` on BBC B, Master 128 / MOS 3.20, Master 128 / MOS 3.50, and Master Compact (with an ADFS image).
